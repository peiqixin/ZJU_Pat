...
/*
	...
	Each input file contains one test case. For each case, the first line contains 3 positive integers, N<=10000​​ ),
	the total number of users, K (≤5), the total number of problems, and M(≤10^​5​​ ), 
	the total number of submissions. 
	It is then assumed that the user id's are 5-digit numbers from 00001 to NN, 
	and the problem id's are from 1 to KK. The next line contains KK positive integers p[i] (i=1, ..., KK), 
	where p[i] corresponds to the full mark of the i-th problem. 
	Then MM lines follow, each gives the information of a submission in the following format:

	user_id problem_id partial_score_obtained
	where partial_score_obtained is either -1−1 if the submission cannot even pass the compiler, 
